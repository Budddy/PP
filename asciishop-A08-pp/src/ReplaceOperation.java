...
/**
 * This Class provides a method to replace all occurrences of a specified char by another specified char. It implements
...
public class ReplaceOperation implements Operation {
	...
	/**
	 * Executes this ReplaceOperation and returns as new AsciiImage where all occurrences of the oldChar are replaced by
	 * the newChar. Other chars remain unchanged.
	 * 
	 * @param img
	...
	public AsciiImage execute(AsciiImage img) throws OperationException {
		...
		if (img.getCharset().indexOf(this.newChar) < 0) {
			throw new OperationException("Invalid char");
		}
		
		AsciiImage result = new AsciiImage(img);
		
		ArrayList<AsciiPoint> region = img.getPointList(this.oldChar);
		for (AsciiPoint p : region) {
			result.setPixel(p, this.newChar);
		}
